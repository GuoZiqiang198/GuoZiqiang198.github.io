...
（注：为便于 Abaqus UMAT 数据传递，本实现采用相应的 Voigt 记法。）
...
> 注：上述推导与 Ghaei 等（Int. J. Mech. Sci. 2010, 52: 531–540）的表述一致，其各向异性张量记号 $\boldsymbol{N}$ 即本文的 $\boldsymbol{P}$；本模型采用功共轭定义 $dp=d\lambda$。
